...
public abstract class MultiTouchObject implements Serializable {
    ...
    protected final transient Paint textPaint = new Paint();
    ...
    protected void init(Resources res) {
        ...
        displayWidth =
                (res.getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE)
                        ...
                        : Math.min(metrics.widthPixels, metrics.heightPixels) / 2; //divide by 2 to reduce size otherwise it will take original size
        displayHeight =
                (res.getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE)
                        ? Math.min(metrics.widthPixels, metrics.heightPixels)
                        : Math.max(metrics.widthPixels, metrics.heightPixels) / 2;
    }
