...
import com.cometchat.pro.uikit.Utility;
 ...
 * @link= "<a href="https://prodocs.cometchat.com/docs/android-ui-unified">...</a>"
 * Created on - 20th December 2019
 *
 * Modified on  - 16th January 2020
 */
public class CometChatUI extends AppCompatActivity implements
        NavigationBarView.OnItemSelectedListener,OnAlertDialogButtonClickListener {
    ...
    /**
     * This method start comet chat dashboard activity
     * @param context context
     * @param notificationData json data in string
     */
    public static void StartCometChatActivity(Context context, String notificationData){
        Intent intent = new Intent(context,CometChatUI.class);
        if(notificationData != null ) {
            Bundle bundle = new Bundle();
            bundle.putString("user_data", notificationData);
            intent.putExtra("data", bundle);
        }
        context.startActivity(intent);
        ((Activity)context).overridePendingTransition(R.anim.anim_from_right, R.anim.anim_from_left);
    }
    ...
    /**
     * Click listener for recent chats items
     */
    ...
    /**
     * Click listener for group chats items
     */
    ...
    /**
     * Click listener for user chats users
     */
    ...
    private void initViewComponent() {
        ...
        activityCometChatUnifiedBinding.bottomNavigation.setOnItemSelectedListener(this);
        ...
        // This decide which tab user want to show by passing parameter
        UIKitSettings.users(AppConfig.DashBoardTabs.IS_USERS);
        UIKitSettings.groups(AppConfig.DashBoardTabs.IS_GROUPS);
        UIKitSettings.conversations(AppConfig.DashBoardTabs.IS_CHATS);
...
//        activityCometChatUnifiedBinding.bottomNavigation.getMenu().add(Menu.NONE,12,Menu.NONE,"Test").setIcon(R.drawable.ic_security_24dp);
        ...
        if(getIntent().hasExtra("data") && getIntent().getExtras() != null){
            try {
                Bundle bundle = getIntent().getExtras().getBundle("data");
                if (bundle == null) return;
                String notificationData = bundle.getString("user_data");
                if(notificationData == null) return;
                BaseMessage baseMessage = CometChatHelper.processMessage(new JSONObject(notificationData));
                if(baseMessage == null) return;

                User user = (User) baseMessage.getSender();
                startUserIntent(user);
    ...
    public void onBackPressed() {
        super.onBackPressed();
        if(CometChatStart.getInstance().getmWebView() != null
                && AppConfig.CALLBACK_FUNCTION != null && AppConfig.CALLBACK_FUNCTION.length() > 0 ){
            Utility.callJavaScriptFunction(this,CometChatStart.getInstance().getmWebView(), AppConfig.CALLBACK_FUNCTION,new JSONObject());
        }
    }
}