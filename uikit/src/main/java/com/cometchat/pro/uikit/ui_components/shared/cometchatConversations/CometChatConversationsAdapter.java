...
public class CometChatConversationsAdapter extends RecyclerView.Adapter<CometChatConversationsAdapter.ConversationViewHolder> implements Filterable {
    ...
    public void onBindViewHolder(@NonNull ConversationViewHolder conversationViewHolder, int position) {
        ...
        if(selectedPosition == position){
            conversationViewHolder.conversationListRowBinding.conversationView.setBackgroundColor(ContextCompat.getColor(context,R.color.grey_200));
        }else {
            conversationViewHolder.conversationListRowBinding.conversationView.setBackgroundColor(ContextCompat.getColor(context,R.color.white));
        }

        if(conversation.getTags() == null){
            conversationViewHolder.conversationListRowBinding.imagePin.setVisibility(View.GONE);
        } else {
            conversationViewHolder.conversationListRowBinding.imagePin.setVisibility(View.VISIBLE);
        }
    ...
    public void update(Conversation conversation) {
        int position = returnChatPosition(conversation);
        if (filterConversationList.contains(conversation)) {
            ...
            filterConversationList.add(position, conversation);
        } else {
            filterConversationList.add(position, conversation);
        }
        notifyDataSetChanged();
    ...
    private int returnChatPosition(Conversation conversations) {

        for (Conversation conversation : filterConversationList) {
            if(conversation.getTags() != null){
                return filterConversationList.indexOf(conversations);
            }
        }
        return 0;
    }
