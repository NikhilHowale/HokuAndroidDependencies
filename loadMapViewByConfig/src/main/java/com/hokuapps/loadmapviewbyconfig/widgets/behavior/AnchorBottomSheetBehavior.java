package com.hokuapps.loadmapviewbyconfig.widgets.behavior;
...
public class AnchorBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    boolean isInsideHeaderOnly = false;

    public boolean isInsideHeaderOnly() {
        return isInsideHeaderOnly;
    }

    public void setInsideHeaderOnly(boolean insideHeaderOnly) {
        isInsideHeaderOnly = insideHeaderOnly;
    }
    ...
    private boolean isTouchInsideHeader(View child, int x, int y) {
        int[] loc = new int[2];

        View overlayHeader = ((CardView) child).getChildAt(0);
        overlayHeader.getLocationInWindow(loc);
        return (x >= loc[0]
                && x <= loc[0] + overlayHeader.getMeasuredWidth()
                && y >= loc[1]
                && y <= loc[1] + overlayHeader.getMeasuredHeight());
...
//        Close overlay if user drags by handle at any situation.
        if (isTouchInsideHeader(child, (int) event.getX(), (int) event.getY())) {
